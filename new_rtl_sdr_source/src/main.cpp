...
private:
    ...
    static void start(void* ctx) {
        ...
        {
            flog::info("RTLSDRSourceModule '{0}': Wrong tuner, only works on r820/r820t2", _this->name);
        }
    ...
    static void menuHandler(void* ctx) {
        ...
        {
            SmGui::LeftLabel("Harmonic Reception");
            SmGui::FillWidth();
            if (ImGui::SliderInt(CONCAT("##_rtlsdr_harmonic_", _this->name), &_this->harmonic_level, 0, 16, _this->harmonic_level_Txt)) 
            {
                // cool slider text
                if (_this->harmonic_level == 0)
                {
                    sprintf(_this->harmonic_level_Txt, "Off"); 
                }
                else 
                {
                    sprintf(_this->harmonic_level_Txt, "%ith" ,_this->harmonic_level);
                }
            

                if (_this->running)
                {
                    rtlsdr_set_harmonic_rx(_this->openDev, _this->harmonic_level);
                }
            }
            
            SmGui::BeginGroup();
            SmGui::Columns(2, CONCAT("RtlsdrTunerModeColumns##_", _this->name), false);
            SmGui::ForceSync();

            // conrol mode 1 exposed
            // control mode 0 regular

            if (SmGui::RadioButton(CONCAT("Auto##_rtlsdr_autotuner_", _this->name), _this->control_mode == 0))
            {
                _this->control_mode = 0;
                if (_this->running) {
                    // disable manual mode and bring back the automatic IF mode
                    rtlsdr_set_tuner_gain_mode(_this->openDev, 1);
                    rtlsdr_set_tuner_if_mode(_this->openDev, 0);
                }
            }
            ...
            if (SmGui::RadioButton(CONCAT("Manual##_rtlsdr_autotuner_", _this->name), _this->control_mode == 1)) 
            {
                _this->control_mode = 1;
                if (_this->running) {
                    // enable manual mode, set IF to manual and apply the manual settings
                    rtlsdr_set_tuner_gain_mode(_this->openDev, 1);
                    rtlsdr_set_tuner_gain_ext(_this->openDev, _this->lnaGain, _this->mixerGain, _this->vgaGain);

                    // apply the the tuner filter settings too.
                    rtlsdr_set_tuner_i2c_register(_this->openDev, 11, 15, _this->lpfCutoff);
                    rtlsdr_set_tuner_i2c_register(_this->openDev, 27, 15<<4, (15 - _this->lpnfCutoff)<<4);
                    rtlsdr_set_tuner_i2c_register(_this->openDev, 27, 15, (15 - _this->hpfCutoff));
                    rtlsdr_set_tuner_i2c_register(_this->openDev, 10, 15, (15 - _this->filterBw));
                }
            }

            SmGui::Columns(1, CONCAT("EndRtlsdrTunerModeColumns##_", _this->name), false);
            SmGui::EndGroup();
        }
        ...
        {
            if (_this->tunerAgc || _this->gainList.size() == 0){ImGui::BeginDisabled();}

            // exposed controls mode
            SmGui::LeftLabel("Lna Gain");
            SmGui::FillWidth();
            if (ImGui::SliderInt(CONCAT("##_rtlsdr_lnagain_", _this->name), &_this->lnaGain, 0, 15, _this->lnaGainTxt)) 
            {
                sprintf(_this->lnaGainTxt, "%i dB", _this->lnaGain);
                if (_this->running){
                rtlsdr_set_tuner_gain_ext(_this->openDev, _this->lnaGain, _this->mixerGain, _this->vgaGain);
                }
            }
            ...
            {
                sprintf(_this->mixerGainTxt, "%i dB", _this->mixerGain);
                if (_this->running){
                rtlsdr_set_tuner_gain_ext(_this->openDev, _this->lnaGain, _this->mixerGain, _this->vgaGain);
                }
            }
            ...
            {
                sprintf(_this->vgaGainTxt, "%i dB", _this->vgaGain);
                if (_this->running){
                rtlsdr_set_tuner_gain_ext(_this->openDev, _this->lnaGain, _this->mixerGain, _this->vgaGain);
                }
            }
            ...
            SmGui::FillWidth();
            if (ImGui::SliderInt(CONCAT("##_rtlsdr_lpfcutoff_", _this->name), &_this->lpfCutoff, 0, 15)) 
            {
                if (_this->running){
                    rtlsdr_set_tuner_i2c_register(_this->openDev, 11, 15, _this->lpfCutoff);
                }
            ...
            SmGui::FillWidth();
            if (ImGui::SliderInt(CONCAT("##_rtlsdr_lpnfcutoff_", _this->name), &_this->lpnfCutoff, 0, 15)) 
            {
                if (_this->running){
                    rtlsdr_set_tuner_i2c_register(_this->openDev, 27, 15<<4, (15 - _this->lpnfCutoff)<<4);
                }
            ...
            SmGui::FillWidth();
            if (ImGui::SliderInt(CONCAT("##_rtlsdr_hpfcutoff_", _this->name), &_this->hpfCutoff, 0, 15)) 
            {
                if (_this->running){
                    rtlsdr_set_tuner_i2c_register(_this->openDev, 27, 15, (15 - _this->hpfCutoff));
                }
            ...
            if (_this->tunerAgc || _this->gainList.size() == 0){ImGui::EndDisabled();}
        ...
        if (SmGui::Checkbox(CONCAT("Tuner AGC##_rtlsdr_tuner_agc_", _this->name), &_this->tunerAgc)) {
            if (_this->running) {
                if (_this->tunerAgc) {
                    rtlsdr_set_tuner_gain_mode(_this->openDev, 0); // 0 means automatic gain mode

                    if (_this->correctTuner)
                    {
                        rtlsdr_set_tuner_if_mode(_this->openDev, 0); // automatic vga
                    }
                }
                else {
                    ...
                    if (_this->correctTuner)
                    {
                        // when Tuner AGC is disabled, return back the automatic IF mode
                        rtlsdr_set_tuner_if_mode(_this->openDev, 0);
                    }

                    rtlsdr_set_tuner_gain(_this->openDev, _this->gainList[_this->gainId]);

                    // or if you were using the manual controls apply those settings back
                    if (_this->control_mode && _this->correctTuner)
                    {
                        rtlsdr_set_tuner_gain_ext(_this->openDev, _this->lnaGain, _this->mixerGain, _this->vgaGain);
                        rtlsdr_set_tuner_i2c_register(_this->openDev, 11, 15, _this->lpfCutoff);
        ...
        if (_this->correctTuner)
        {
            if (SmGui::Checkbox(CONCAT("Dithering##_rtlsdr_dither_", _this->name), &_this->dither)) {
                if (_this->running) {
                    rtlsdr_set_dithering(_this->openDev, _this->dither);
                }
            }
        }
    ...
    int hpfCutoff = 0;
    int filterBw = 0;
    int harmonic_level = 0;
    char lnaGainTxt[128];
    char mixerGainTxt[128];
    char vgaGainTxt[128];
    char harmonic_level_Txt[128];
